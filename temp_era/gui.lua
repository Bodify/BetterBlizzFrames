...
local function CreateTooltipTwo(widget, title, mainText, subText, anchor, cvarName, category)
    ...
    widget:SetScript("OnEnter", function(self)
        ...
        if category then
            GameTooltip:AddLine("")
            GameTooltip:AddLine(string.format("|A:shop-games-magnifyingglass:17:17|a" .. L["Setting located in section"], category), 0.4, 0.8, 1, true)
        end
        GameTooltip:Show()
    end)
    widget:SetScript("OnLeave", function(self)
...
local function notWorking(element, re)
    ...
    CreateTooltipTwo(element, L["Not Working"], L["Currently not working and disabled for Era. May or may not be removed TBD."], L["A lot of other features might also not work 100% in this Alpha version. Keep an eye out for errors."])
...
local function CreateCheckbox(option, label, parent, cvarName, extraFunc)
    ...
    checkBox.Text:SetFont("Interface\\AddOns\\BetterBlizzFrames\\media\\zhCNFont1.ttf", 11)
...
local function guiGeneralTab()
    ...
    alpha2:SetText(L["BetterBlizzFrames Era is in Alpha. Please report bugs."])
    ...
    local playerFrameOCDTextureBypass = CreateCheckbox("playerFrameOCDTextureBypass", L["OCD: Skip Bars"], BetterBlizzFrames, nil, BBF.HideFrames)
    playerFrameOCDTextureBypass:SetPoint("LEFT", playerFrameOCD.text, "RIGHT", 0, 0)
    CreateTooltip(playerFrameOCDTextureBypass, L["If healthbars & manabars look weird enable this to skip\nadjusting them and only fix portraits + Name Background"])
    ...
    local function OpenClassSpecificWindow()
        if not classOptionsFrame then
            ...
            local classes = {
                ...
                { class = L["Monk"], var = "hidePlayerPowerNoMage", color = RAID_CLASS_COLORS["MAGE"] },
            }
...
--[=[
    ...
    CreateTooltipTwo(targetAndFocusArenaNames, L["Arena Names"],L["Change Target & Focus name to arena ID and/or spec name during arena"], L["Will enable a fake name. Because of this other addons like HealthBarColor's name stuff will not work properly."], "ANCHOR_LEFT")
    ...
    CreateTooltipTwo(partyArenaNames, L["Arena Names"], L["Change party frame names to party ID and/or spec name during arena"],L["Will enable a fake name. Because of this other addons like HealthBarColor's name stuff will not work properly."], "ANCHOR_LEFT")
    ...
    local overShields = CreateCheckbox("overShields", L, BetterBlizzFrames)
    ...
    nahjProfileButton:SetText(L["Nahj Profile"])
...
local function guiMisc()
    ...
    local hunterFeignRaidFrameFix = CreateCheckbox("hunterFeignRaidFrameFix", L["Hunter Feign Fix"], guiMisc, nil, BBF.HunterFeignRaidFrameFix)
    hunterFeignRaidFrameFix:SetPoint("TOPLEFT", useMiniFocusFrame, "BOTTOMLEFT", 0, pixelsBetweenBoxes)
    CreateTooltipTwo(hunterFeignRaidFrameFix, L["Hunter Feign Fix"], L["Don't show Hunters as Dead when using Feign Death to avoid Hardcore heartattacks."])
...
function BBF.CreateIntroMessageWindow()
    ...
    myProfileButton:SetText(L["Starter Profile"])
    ...
    myProfileButton:SetScript("OnClick", function()
        ShowProfileConfirmation(L["Starter Profile"], BBF.StarterProfile)
    end)
    CreateTooltipTwo(myProfileButton, L["Starter Profile"], L["A basic starter profile that only enables the few things you need. Intended to work as a very minimal quick start that can be built upon."])
    ...
    button2:SetText(L["|A:groupfinder-icon-class-druid:16:16|a |cffff7d0aSnupy Profile|r"])
    ...
    button2:SetScript("OnClick", function()
        ShowProfileConfirmation(L["Snupy Profile"], BBF.SnupyProfile)
    end)
    CreateTooltipTwo(button2, L["|A:groupfinder-icon-class-druid:16:16|a |cffff7d0aSnupy Profile|r"], "www.twitch.tv/snupy")
    ...
    CreateTooltipTwo(buttonLast, L["Exit, No Profile"], L["Exit and customize everything yourself."])
