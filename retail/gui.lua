...
local function CreateSimpleDropdown(name, parentFrame, labelText, settingKey, optionsTable, toggleFunc, point, dropdownWidth)
    ...
    local label = container:CreateFontString(nil, "OVERLAY", "GameFontNormalSmall2")
    label:SetPoint("LEFT", container, "LEFT", -50, -12)
    label:SetText(labelText) -- labelText comes from parameter
    label:SetFont(fontMedium, 13)
...
StaticPopupDialogs["BBF_CONFIRM_RELOAD"] = {
    text = titleText..L["Tooltip_Requires_Reload"],
    button1 = L["Yes"],
    button2 = L["No"],
    OnAccept = function()
...
local function CreateSlider(parent, label, minValue, maxValue, stepValue, element, axis, sliderWidth)
    ...
    slider.Text:SetFont(fontMedium, 11)
...
local function CreateTooltipTwo(widget, title, mainText, subText, anchor, cvarName, cpuUsage, category)
    ...
    widget:SetScript("OnEnter", function(self)
        ...
        if title == "Format Numbers" then
            local tooltipText = L["Format_Numbers"]
            if BetterBlizzFramesDB.formatStatusBarTextExtraDecimals then
                tooltipText = "|A:glueannouncementpopup-arrow:20:20|a" .. L["Tooltip_Format_Numbers_Extra"] .. "|A:ParagonReputation_Checkmark:15:15|a"
            end
        ...
        if title == L["Custom_Colors"] then
            ...
            tooltipText = tooltipText .. "\n\n" .. green .. L["Ctrl_Right_Click_Keep_PlayerFrame_Green"] .. reset
            if BetterBlizzFramesDB.classColorFramesSkipPlayer then
                tooltipText = tooltipText .. check
            end

            tooltipText = tooltipText .. "\n\n" .. babyBlue .. L["Shift_Right_Click_Keep_Friendly_Units_Green"] .. reset
            if BetterBlizzFramesDB.classColorFramesSkipFriendly then
                tooltipText = tooltipText .. check
            end
...
local function CreateCheckbox(option, label, parent, cvarName, extraFunc)
    ...
    checkBox.Text:SetFont(fontMedium, 12)
        ...
        for i, spellID in ipairs(fullList) do
            ...
            if info then
                ...
                if not button then
                    ...
                    CreateTooltipTwo(checkbox, L["Tooltip_Hide_Spell_Icon"], L["Tooltip_Hide_Spell_Icon_CD"], nil, "ANCHOR_TOPRIGHT")
        ...
        for _, data in ipairs(checkBoxList) do
            ...
            if matchesQuery(label) or matchesQuery(tooltipTitle) or matchesQuery(tooltipMainText) or matchesQuery(tooltipSubText) or matchesQuery(tooltipCVarName) then
                ...
                if not resultCheckBox then
                    ...
                    resultCheckBox.Text:SetFont(fontMedium, 12)
...
local function guiGeneralTab()
    ...
    do
        ...
        BBF.generalScrollChild:SetSize(650, 1400)
...
--[=[
    ...
    local hidePartyNames = CreateCheckbox("hidePartyNames", L["Hide_Names"], BBF.generalScrollChild, nil, BBF.AllNameChanges)
    ...
    newRaidFrameRoleIcons:SetPoint("TOPLEFT", hideRaidFrameContainerBorder, "BOTTOMLEFT", 0, pixelsBetweenBoxes)
    ...
    partyFrameScale:SetPoint("TOPLEFT", newRaidFrameRoleIcons, "BOTTOMLEFT", 90, -14)
    ...
    targetFrameText:SetPoint("TOPLEFT", mainGuiAnchor, "BOTTOMLEFT", 0, -820)
    ...
    targetToTFrameText:SetPoint("TOPLEFT", mainGuiAnchor, "BOTTOMLEFT", 0, -970)
    ...
    chatFrameText:SetPoint("TOPLEFT", mainGuiAnchor, "BOTTOMLEFT", 0, -1135)
