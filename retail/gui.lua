...
StaticPopupDialogs["BBF_CONFIRM_PVP_BLACKLIST"] = {
    text = L["This will import a large PvP blacklist focused on removing trash buffs, created by me.\nIt will only add auras you don't already have in your blacklist.\n\nAre you sure you want to continue?"],
    button1 = L["Yes"],
    button2 = L["No"],
    OnAccept = function()
...
local function FormatClassName(classTag)
    ...
    return classTag:sub(1, 1):upper() .. classTag:sub(2):lower()
end
...
local function guiProfiles()
    ...
    frame.descriptionText:SetPoint("TOP", frame, "TOP", 2, -30)
    frame.descriptionText:SetText(L["Pre-configured profiles for BetterBlizzFrames:"])
    frame.descriptionText:SetWidth(110)
...
local function guiGeneralTab()
    ...
    midnightBeta:SetText(L["|T136221:12:12|t |cffcc66ffBetterBlizzFrames is now available on Midnight Beta."])
...
--[=[
    ...
    local hidePartyNames = CreateCheckbox("hidePartyNames", L["Hide Names"], BetterBlizzFrames, nil, BBF.AllNameChanges)
    ...
    CreateTooltipTwo(hideFocusToTName, L["Hide Target of Focus Frame Name"])
...
local function guiFrameAuras()
    ...
    CreateTooltipTwo(playerAuraFiltering, L["Enable Buff Filtering & Aura settings"], L["Enables all the buff filtering settings.\nThis setting is cpu heavy and un-optimized so use at your own risk."], nil, nil, nil, 5)
...
--[[targetBuffPurgeGlow
    ...
    CreateTooltipTwo(PlayerAuraFrameBuffEnable, L["Show Player Buffs"], L["Show Player Buffs (Top Right)"], L["If disabled all filtering will be skipped and instead just hide the BuffFrame entirely."])
    ...
    CreateTooltipTwo(PlayerAuraFramedeBuffEnable, L["Show Player Debuffs"], L["Show Player Debuffs (Top Right)"], L["If disabled all filtering will be skipped and instead just hide the DebuffFrame entirely."])
...
--[=[
    ...
    local playerAuraSpacingX = CreateSlider(playerAuraFiltering, L["Horizontal Padding"], 0, 10, 1, "playerAuraSpacingX", "X")
    ...
    useEditMode:SetText(L["Use Edit Mode for other settings."])
    ...
    local showPurgeTextureOnSelf = CreateCheckbox("showPurgeTextureOnSelf", L["Show Purge Texture on Player Auras"], playerAuraFiltering)
    showPurgeTextureOnSelf:SetPoint("TOPLEFT", changePurgeTextureColor, "BOTTOMLEFT", 0, pixelsBetweenBoxes)
    CreateTooltipTwo(showPurgeTextureOnSelf, L["Show Purge Texture on Player Auras"], L["Show Purge Texture on Player Auras (Top Right)."])
    ...
    CreateTooltipTwo(clickthroughAuras, L["Clickthrough Auras"], L["Makes auras on target and focus frame clickthrough.\n\nNote: This setting will make it so can no longer click auras to whitelist & blacklist them."])
    ...
    local auraImportantDispelIcon = CreateCheckbox("auraImportantDispelIcon", L["Important Glow: Dispel Icon"], playerAuraFiltering)
    auraImportantDispelIcon:SetPoint("TOPLEFT", clickthroughAuras, "BOTTOMLEFT", 0, pixelsBetweenBoxes)
    CreateTooltipTwo(auraImportantDispelIcon, L["Important Glow: Dispel Icon"], L["If an aura is marked as Important and has a glow, this setting adds a blue exclamation mark in the bottom left corner if the aura is dispellable.\n\nSince the Important Glow hides the default dispellable glow, this helps you quickly see if an aura can be dispelled (especially for auras that have both dispellable and non-dispellable versions)."])
...
local function guiMisc()
    ...
    local druidOverstacks = CreateCheckbox("druidOverstacks", L["Druid: Color Berserk Overstack Combo Points Blue"], guiMisc)
    druidOverstacks:SetPoint("TOPLEFT", surrenderArena, "BOTTOMLEFT", 0, pixelsBetweenBoxes)
    CreateTooltipTwo(druidOverstacks, L["Druid: Color Berserk Overstack Combo Points Blue"], L["Color the Druid Berserk Overstack Combo Points blue similar to Rogue's Echoing Reprimand."])
...
local function guiCooldownManager()
    ...
    guiCdManager.name = L["CD Manager"]--"|A:GarrMission_CurrencyIcon-Material:19:19|a Misc"
    ...
    settingsText:SetText(L["Cooldown Manager"])
    ...
    cdNote:SetFont("Interface\\AddOns\\BetterBlizzFrames\\media\\zhCNFont1.ttf", 11)
    cdNote:SetText(L["WIP: Early version. Reloads between changes might be needed. Expect this to get many tweaks in the next few weeks. Please report any bugs."])

    local list = CreateCDManagerList(guiCdManager)

    local cooldownViewerEnabled = CreateCheckbox("cooldownViewerEnabled", L["Enable Cooldown Manager"], guiCdManager, "cooldownViewerEnabled")
    ...
    CreateTooltipTwo(cooldownViewerEnabled, L["Enable Cooldown Manager"], L["Enable Blizzard's new Cooldown Manager introduced in 11.1.5"])
    cooldownViewerEnabled:SetChecked(C_CVar.GetCVarBool("cooldownViewerEnabled"))

    local cdManagerSorting = CreateCheckbox("cdManagerSorting", L["Filter & Sort Icons"], guiCdManager, nil, BBF.HookCooldownManagerTweaks)
    cdManagerSorting:SetPoint("TOPLEFT", cooldownViewerEnabled, "BOTTOMLEFT", 0, pixelsBetweenBoxes)
    CreateTooltipTwo(cdManagerSorting, L["Filter & Sort Icons"], L["Filter and sort icons on the Cooldown Manager."])
    list:SetAlpha(BetterBlizzFramesDB.cdManagerSorting and 1 or 0.3)


    local cdManagerCenterIcons = CreateCheckbox("cdManagerCenterIcons", L["Center Icons"], guiCdManager, nil, BBF.HookCooldownManagerTweaks)
    cdManagerCenterIcons:SetPoint("TOPLEFT", cdManagerSorting, "BOTTOMLEFT", 0, pixelsBetweenBoxes)
    CreateTooltipTwo(cdManagerCenterIcons, L["Center Icons"], L["Center icons on the Cooldown Manager"])
