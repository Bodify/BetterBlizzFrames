    ...
    widget:SetScript("OnEnter", function(self)
        ...
        if title == "Format Numbers" then
            ...
            if BetterBlizzFramesDB.formatStatusBarTextExtraDecimals then
                tooltipText = L["Tooltip_Format_Numbers_Extra"]
            end
...
local function guiGeneralTab()
    ...
    if useCustomFonts then
        ...
        local gameFont = GameFontNormal:GetFont()
        midnightBeta:SetFont(gameFont, 24, L["THINOUTLINE"])
    end
    midnightBeta:Hide()
...
--[=[
    ...
    CreateTooltipTwo(singleValueStatusBarText, L["No_Max_Value"], L["Tooltip_No_Max_Desc"], L["Requires_Reload"])
...
local function guiMisc()
    ...
    CreateTooltipTwo(minimizeObjectiveTracker, L["Minimize_Objective_Better"], L["Tooltip_Minimize_Objective"])
