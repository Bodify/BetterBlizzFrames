...
function BBF.UpdateTipOfSpearBar()
    ...
    local show = BetterBlizzFramesDB.hunterTipOfSpearCombos
        and GetSpecialization() == SURVIVAL_SPEC_INDEX and not LegacyOwnsDisplay()
...
function BBF.CreateTipOfSpearBar()
    ...
    if GetSpecialization() ~= SURVIVAL_SPEC_INDEX then return end
...
function BBF.HunterLegacyTipOfSpear()
    ...
    local function UpdateHunterLegacyCombo()
        ...
        local stacks = GetSpecialization() == SURVIVAL_SPEC_INDEX and GetStacks() or 0
