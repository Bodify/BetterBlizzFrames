...
function BBF.SymmetricPlayerFrame()
    ...
    if BBF.isMidnight then
        C_Timer.After(5, function()
            print("BBF: " .. L["Print_Symmetric_Disabled_Midnight"])
        end)
        return
    end
...
Frame:SetScript("OnEvent", function(...)
    ...
    C_Timer.After(3.5, function()
        print("|A:gmchat-icon-blizz:16:16|a Better|cff00c0ffBlizz|rFrames: " .. L["Print_Bugs_Expected"])
    end)
...
First:SetScript("OnEvent", function(_, event, addonName)
    if addonName == "BetterBlizzFrames" then
        ...
        if not BetterBlizzFramesDB.cleanedAuraBlacklist then
            ...
            if removedAura then
                C_Timer.After(3, function()
                    DEFAULT_CHAT_FRAME:AddMessage("|A:gmchat-icon-blizz:16:16|aBetter|cff00c0ffBlizz|rFrames: " .. L["Print_Removed_PvP_Blacklist_Auras"])
                end)
            end
        end
