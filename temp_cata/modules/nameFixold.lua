...
--             local memberFrame = _G["CompactRaidFrame" .. i]
...
local function checkWeight()
    ...
    if focusExists and classColorFocusReputationTexture then
        BBF.ClassColorReputation(FocusFrameNameBackground, "focus")
    end

    if BetterBlizzFramesDB.biggerHealthbars then
        local nameBackground = _G["TargetFrameNameBackground"]
        local background = _G["TargetFrameBackground"]
...
function BBF.ClassColorPlayerName()
    ...
    if BetterBlizzFramesDB.classColorTargetNames then
        ...
        PlayerFrame.name:SetTextColor(1, 0.81960791349411, 0)
...
function BBF.OnUpdateName()
    local groupMembers = GetNumGroupMembers()
    for i = 1, groupMembers + 1 do
        local compactPartyMember = _G["CompactRaidFrame" .. i]
        local roleIcon = _G["CompactRaidFrame" .. i .. "RoleIcon"]
        local defaultMember = _G["PartyMemberFrame" .. i]

        if compactPartyMember and compactPartyMember:IsShown() then
            -- Hide the role icon if hidePartyRoles is true
            if hidePartyNames then
                if compactPartyMember.name then
                    compactPartyMember.name:SetAlpha(0)
                end
                if not partyArenaNames and compactPartyMember.cleanName then
...
local partyFrames = {
    ...
    ["CompartPartyFrameMember1"] = true,
    ["CompartPartyFrameMember2"] = true,
    ["CompartPartyFrameMember3"] = true,
    ["CompartPartyFrameMember4"] = true,
    ["CompartPartyFrameMember5"] = true,
}
...
local function hideName(frame, hide)
    if hide then
        frame.name:SetAlpha(0)
    end
end
...
local unitFrameNameHooked
function BBF.HookUnitFrameName()
    ...
    if not unitFrameNameHooked then
        hooksecurefunc("UnitFrame_Update", function(frame)
            ...
            if not frame then return end
            local frameName = frame:GetName()
            local playerFrame = frameName == "PlayerFrame"
            local targetFrame = frameName == "TargetFrame"
            local focusFrame = frameName == "FocusFrame"
            local targetToTFrame = frameName == "TargetFrameToT"
            local focusToTFrame = frameName == "FocusFrameToT"
        
            if playerFrame and hidePlayerName then
                frame.name:SetAlpha(0)
            elseif targetFrame and hideTargetName then
                frame.name:SetAlpha(0)
            elseif focusFrame and hideFocusName then
                frame.name:SetAlpha(0)
            elseif targetToTFrame and hideTargetToTName then
                frame.name:SetAlpha(0)
            elseif focusToTFrame and hideFocusToTName then
                frame.name:SetAlpha(0)
            end
        
            -- if hideNames2[frameName] then
            --     hideName(frame, BetterBlizzFramesDB.hidePlayerName)
            -- end
        end)
        unitFrameNameHooked = true
    end
end